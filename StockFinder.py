...
NSE_OPTION_CHAIN_NIFTY = "https://www.nseindia.com/api/option-chain-indices?symbol=NIFTY"
...
def swing_highs_lows(df: pd.DataFrame, lookback=5) -> Tuple[pd.Series, pd.Series]:
    ...
    highs = (df['High']
             ...
             .apply(lambda w: w[lookback//2] == max(w), raw=True)).fillna(0).astype(bool)
    lows = (df['Low']
            .rolling(lookback, center=True)
            .apply(lambda w: w[lookback//2] == min(w), raw=True)).fillna(0).astype(bool)
    return highs, lows

# -------------- News --------------
def fetch_news_newsapi(query: str, since: dt.datetime, api_key: Optional[str]) -> List[Dict]:
...
def fetch_news_gdelt(query: str, since: dt.datetime) -> List[Dict]:
    ...
    try:
        ...
        for a in arts:
            ...
            try:
                ts = dt.datetime.strptime(a.get("seendate")[:14], "%Y%m%d%H%M%S").replace(tzinfo=dt.timezone.utc)
            except:
                ts = None
...
def finbert_pipeline():
    ...
    try:
        ...
        _finbert = TextClassificationPipeline(model=mdl, tokenizer=tok, return_all_scores=False)
        return _finbert
...
def sentiment_label(text: str, prefer_finbert=True) -> Tuple[str, float]:
    ...
    if prefer_finbert:
        ...
        if pipe:
            try:
                out = pipe(text[:512])[0]  # {'label': 'positive'|'negative'|'neutral', 'score': float}
                return (out['label'].lower(), float(out['score']))
            except:
                pass
    s = _sia.polarity_scores(text)
    comp = s["compound"]
    if comp >= 0.2: return ("positive", comp)
    if comp <= -0.2: return ("negative", comp)
    return ("neutral", comp)
...
def get_nifty_pcr_quick() -> Optional[float]:
    ...
    try:
        ...
        put_oi = 0; call_oi = 0
        for row in recs:
            if "CE" in row: call_oi += row["CE"].get("openInterest", 0) or 0
            if "PE" in row: put_oi += row["PE"].get("openInterest", 0) or 0
        if call_oi == 0: return None
...
def fetch_prices(symbol_yf: str, days=3) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    intraday = yf.download(symbol_yf, period="5d", interval="15m", progress=False, auto_adjust=True)
    daily = yf.download(symbol_yf, period="2y", interval="1d", progress=False, auto_adjust=True)
    # trim intraday to last N days (IST sessions)
    if len(intraday) > 0:
        cutoff = now_ist() - dt.timedelta(days=days)
        intraday = intraday[intraday.index.tz_localize(None) >= (cutoff.replace(tzinfo=None))]
    return intraday, daily

def compute_indicators(intra: pd.DataFrame, daily: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    if len(intra)==0 or len(daily)==0: return intra, daily
    # intraday indicators
    intra = intra.copy()
    ...
    daily["HH"] = (daily["High"] > daily["High"].shift(1)) & (daily["Low"] > daily["Low"].shift(1))  # crude
    return intra, daily

def breakout_retest_levels(daily: pd.DataFrame) -> Tuple[Optional[float], Optional[float]]:
    # Simple: use last 20d high as resistance; if close > that, breakout. Entry at retest ~ that level.
    if len(daily) < 50: return None, None
...
def classify(intra: pd.DataFrame, daily: pd.DataFrame) -> Tuple[str, str, Optional[float], Optional[float], Optional[float], Optional[float]]:
    """
    Returns: (label, reason, entry, stop, target, rrr)
    Implements your rule-book conservatively.
    """
    if len(intra)==0 or len(daily)==0: return ("Watch", "No data", None, None, None, None)
    close = daily["Close"].iloc[-1]
    s50 = daily["SMA50"].iloc[-1]
    s200 = daily["SMA200"].iloc[-1]

    # Trend structure
    higher_trend = close > s50 and close > s200 and (daily["Close"].iloc[-5:] > daily["Close"].shift(1).iloc[-5:]).sum() >= 3
    lower_trend  = close < s50 and close < s200 and (daily["Close"].iloc[-5:] < daily["Close"].shift(1).iloc[-5:]).sum() >= 3

    # Crossovers
    golden = daily["SMA50"].iloc[-1] > daily["SMA200"].iloc[-1] and daily["SMA50"].iloc[-2] <= daily["SMA200"].iloc[-2]
    death  = daily["SMA50"].iloc[-1] < daily["SMA200"].iloc[-1] and daily["SMA50"].iloc[-2] >= daily["SMA200"].iloc[-2]
    ...
    strong_buy = False; strong_sell = False; reason = []
    ...
    # Buy case
    if higher_trend and vol_ok and rsi >= 50 and (not np.isnan(macd_hist) and macd_hist > 0):
        # Breakout check
        if res and daily["Close"].iloc[-1] > res:
            ...
            entry = float(res)
            stop = float(entry - 1.5 * atr) if not math.isnan(atr) else float(entry * 0.98)
            target = float(entry + 2.0 * atr) if not math.isnan(atr) else float(entry * 1.03)
            rrr = (target-entry)/max(1e-6,(entry-stop))
            strong_buy = True
            reason.append("Breakout + bullish retest setup")
            return ("Strong Buy", ", ".join(reason), entry, stop, target, safe_round(rrr,2))
    ...
    if lower_trend and rsi <= 50 and (not np.isnan(macd_hist) and macd_hist < 0):
        if sup and daily["Close"].iloc[-1] < sup:
            atr = ta.atr(daily["High"], daily["Low"], daily["Close"], length=14).iloc[-1]
            entry = float(sup)
            stop = float(entry + 1.5 * atr) if not math.isnan(atr) else float(entry * 1.02)
            target = float(entry - 2.0 * atr) if not math.isnan(atr) else float(entry * 0.97)
            rrr = (entry-target)/max(1e-6,(stop-entry))
            strong_sell = True
            reason.append("Breakdown + failed retest setup")
            return ("Strong Sell", ", ".join(reason), entry, stop, target, safe_round(rrr,2))

    return ("Watch", ", ".join(reason) if reason else "Mixed/No confirmation",
            None, None, None, None)

def trend_last_n_days(daily: pd.DataFrame, n=3) -> str:
    if len(daily) < n+1: return "flat"
    ret = pct(daily["Close"].iloc[-1], daily["Close"].iloc[-n-1])
    if ret is None or math.isnan(ret): return "flat"
    if ret > 1.5: return "continuously_increasing"
    if ret < -1.5: return "continuously_decreasing"
    return "flat"

def analyze_symbol(row, days=3, prefer_finbert=True) -> Optional[SignalResult]:
    name = row["Company Name"]; sym = row["Symbol"]; yf_sym = row["YF"]
    ...
    if pos>neg: news_tag = f"positive ({pos} vs {neg})"
    elif neg>pos: news_tag = f"negative ({neg} vs {pos})"
    else: news_tag = "mixed/neutral"
    ...
    last_macdh = safe_round(intra["MACDhist"].iloc[-1],3) if "MACDhist" in intra else None
    last_stochk = safe_round(intra["STOCHK"].iloc[-1],1) if "STOCHK" in intra else None

    return SignalResult(
        symbol=sym, name=name, strong=label, reason=reason,
        entry=entry, stop=stop, target=target,
        rrr=rrr, rsi=last_rsi, macd_hist=last_macdh, stoch_k=last_stochk,
        news_impact=news_tag, news_links=links
    )

def main():
    load_dotenv()
    ap = argparse.ArgumentParser()
    ap.add_argument("--days", type=int, default=3, help="Lookback window for news & trend notes")
    ap.add_argument("--max-stocks", type=int, default=50, help="Limit scan for speed while testing")
    ap.add_argument("--make-report", action="store_true", help="Save markdown report & CSV")
    ap.add_argument("--finbert", action="store_true", help="Prefer FinBERT sentiment (needs transformers)")
    args = ap.parse_args()
    ...
    def rowfmt(r: SignalResult):
        return [
            r.symbol, r.strong, safe_round(r.entry,2), safe_round(r.stop,2),
            safe_round(r.target,2), safe_round(r.rrr,2),
            r.rsi, r.macd_hist, r.stoch_k, r.news_impact
        ]
    cols = ["Symbol","Signal","Entry","SL","Target","RRR","RSI","MACD_hist","StochK","News"]
    ...
    df_all = pd.DataFrame([{
        "symbol": r.symbol, "name": r.name, "signal": r.strong, "reason": r.reason,
        "entry": r.entry, "stop": r.stop, "target": r.target, "rrr": r.rrr,
        "rsi": r.rsi, "macd_hist": r.macd_hist, "stoch_k": r.stoch_k,
        "news": r.news_impact, "news_links": " | ".join(r.news_links)
    } for r in results])
    df_all.to_csv(f"signals_last{args.days}d.csv", index=False)

    if args.make_report:
        lines = []
        lines.append(f"# NIFTY 50 Signals — last {args.days} days (IST: {now_ist().strftime('%Y-%m-%d %H:%M')})")
        # Market context (optional PCR)
        pcr = get_nifty_pcr_quick()
        if pcr:
            ctx = "Overbought/Caution" if pcr>1.3 else ("Oversold/Bounce-risk" if pcr<0.7 else "Neutral")
            lines.append(f"\n**NIFTY PCR** ≈ {pcr:.2f} → {ctx}")
        lines.append("\n## Strong Buy")
        for r in buys:
            lines.append(f"- **{r.symbol}** — {r.reason} | Entry ~ **{safe_round(r.entry,2)}**, SL **{safe_round(r.stop,2)}**, Target **{safe_round(r.target,2)}** (RRR {r.rrr}); RSI {r.rsi}, MACDh {r.macd_hist}. News: {r.news_impact}")
        lines.append("\n## Strong Sell")
        for r in sells:
            lines.append(f"- **{r.symbol}** — {r.reason} | Short near **{safe_round(r.entry,2)}**, SL **{safe_round(r.stop,2)}**, Target **{safe_round(r.target,2)}** (RRR {r.rrr}); RSI {r.rsi}, MACDh {r.macd_hist}. News: {r.news_impact}")
        lines.append("\n## Notes")
        lines.append("- Entry levels use breakout/breakdown retest logic from prior 20-day swing & ATR-based SL/targets.")
        lines.append("- Signals require **trend (50/200-DMA), volume, RSI>50 or <50, MACD histogram sign** to align.")
        lines.append("- News is tagged **positive/negative/mixed** and whether price is **continuously increasing/decreasing** over the last 3 days.")
