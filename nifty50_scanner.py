...
def trend_structure(df: pd.DataFrame) -> str:
    ...
    if (
        last["Close"]
        > last.get("SMA50", last["Close"])
        > last.get("SMA200", last["Close"])
    ):
        return "uptrend"
    if (
        last["Close"]
...
def derive_signal(df15: pd.DataFrame, sentiment_score: float) -> Tuple[str, int]:
    ...
    # RSI
    if safe_float(last["RSI"], 50) < 40:
        score += 1
    elif safe_float(last["RSI"], 50) > 70:
        score -= 1
...
def entry_exit(
    ...
    last = df15.iloc[-1]
    cond_breakout = last["Close"] > swing_high and last["Volume"] > safe_float(
        last.get("VolMA20"), 0
    )
    if cond_breakout and last["Close"] > last.get("EMA20", last["Close"]):
        ...
        risk = max(
            0.05 * entry, entry - stop
        )  # guard: at least 5% of price if swing too tight
        ...
        if last["Close"] > last.get("EMA50", last["Close"]) and last.get(
            "EMA20", last["Close"]
        ) > last.get("EMA50", last["Close"]):
...
def main():
    ...
    parser.add_argument(
        "--model", default="gpt-4o-mini", help="OpenAI model (e.g., gpt-4o-mini)"
    )
    parser.add_argument(
    ...
    for sym in tqdm(symbols):
        try:
            res = analyze_symbol(
                sym, model=args.model, days=args.days, make_charts=args.charts
            )
            rows.append(res)
        except Exception as e:
    ...
    df.sort_values(
        ["signal", "score"], ascending=[True, False], inplace=True, na_position="last"
    )
