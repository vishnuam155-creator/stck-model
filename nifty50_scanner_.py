...
def fetch_ohlcv(symbol: str, days: int = 3, interval: str = "15m") -> pd.DataFrame:
    df = yf.download(
        symbol, period=f"{days}d", interval=interval, progress=False, auto_adjust=True
    )
...
# --------------------------- News --------------------------- #
def news_from_newsapi(
    query: str, days: int = 3, page_size: int = 10
) -> List[Dict[str, Any]]:
    if not NEWS_API_KEY:
        return []
...
def news_from_google(
    query: str, days: int = 3, limit: int = 10
) -> List[Dict[str, Any]]:
...
def get_latest_news(
    symbol: str, company_hint: Optional[str] = None, days: int = 3, limit: int = 12
):
...
def openai_classify_headlines(
    headlines: List[str], model: str = "gpt-4o-mini"
) -> List[Dict[str, Any]]:
    ...
    if to_query:
        if not OPENAI_API_KEY:
            raise RuntimeError(
                "OPENAI_API_KEY missing. Use --sentiment finbert or set the key."
            )
        ...
        user = "Headlines:\n" + "\n".join(
            [f"- {h}" for h in to_query if isinstance(h, str)]
        )
        headers = {
...
def finbert_classify_headlines(
    ...
    for h in headlines:
        key = md5(h.strip().lower())
        (out if (key in cache and "impact" in cache[key]) else to_run).append(
            cache.get(key, h)
        )
    if to_run:
        fb = finbert_load()
...
def aggregate_sentiment(items: List[Dict[str, Any]]) -> Tuple[float, Dict[str, Any]]:
    ...
    top = sorted(items, key=lambda x: abs(safe_float(x["impact"], 0.0)), reverse=True)[
        :3
    ]
...
def entry_exit(
    ...
    cond_breakout = (row_scalar(last, "Close") > swing_high) and (
        row_scalar(last, "Volume") > vol_ma
    )
...
def daily_ma_flags(symbol: str, lookback_days: int = 400) -> Dict[str, Optional[bool]]:
    try:
        ...
        return {
            "above_50dma": (np.isfinite(close) and np.isfinite(sma50) and close > sma50)
            or False,
            "above_200dma": (
                np.isfinite(close) and np.isfinite(sma200) and close > sma200
            )
            or False,
        }
    except Exception:
...
def analyze_symbol(
    ...
    news_items = get_latest_news(
        company_hint, company_hint=company_hint, days=days, limit=12
    )
    ...
    if make_charts:
        try:
            ...
            fig.update_layout(
                title=f"{symbol} 15m", xaxis_title="Time", yaxis_title="Price"
            )
    ...
    macds = row_scalar(last, "MACDS")
    macd_delta = (
        (macd - macds) if (np.isfinite(macd) and np.isfinite(macds)) else np.nan
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="NIFTY50 3-day scanner: news + technicals."
    )
    parser.add_argument(
        "--sentiment",
        choices=["finbert", "openai"],
        default="finbert",
        help="Sentiment engine",
    )
    parser.add_argument(
        "--model", default="gpt-4o-mini", help="OpenAI model (if --sentiment openai)"
    )
    parser.add_argument(
        "--days", type=int, default=3, help="Lookback days for prices/news"
    )
    parser.add_argument(
        "--limit", type=int, default=50, help="How many symbols from NIFTY50 to scan"
    )
    ...
    rows = []
    logging.info(
        "Scanning %d symbols over %d days using %s sentiment...",
        len(symbols),
        args.days,
        args.sentiment,
    )
    ...
    for sym in tqdm(symbols):
        try:
            res = analyze_symbol(
                sym,
                sentiment_engine=args.sentiment,
